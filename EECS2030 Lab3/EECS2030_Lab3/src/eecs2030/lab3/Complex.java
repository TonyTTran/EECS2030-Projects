...
public final class Complex {
	...
	public Complex multiply(Complex c) {
		double r2 = this.real * c.real + (-1) * this.img * c.img;

		double i2 = this.img * c.real + c.img * this.real;
	...
	@Override
	public int hashCode() {
		double rp = this.real * 9999;
		double ip = this.img * 99;
	...
	public boolean equals(Object obj) {
		if (obj != null && this.getClass() == obj.getClass()) {
			...
			if (temp.real == this.real && this.img == temp.img) {
				return true;
	...
	public static Complex valueOf(String s) {
		...
		if (parts.size() == 3 && (parts.get(1).equals("+") || parts.get(1).equals("-")) && parts.get(2).endsWith("i")) {
			parts.set(2, parts.get(2).substring(0, parts.get(2).length() - 1));
			double rs = Double.valueOf(parts.get(0));
			double is = Double.valueOf(parts.get(2));

			if (parts.get(1).equals("+")) {
				Complex answer = new Complex(rs, is);
				return answer;
